...
class TaskDecomposer {
  static async decomposeTask(instruction, vehicleId = null) {
    ...
    if (lowerInstruction.includes('load') && lowerInstruction.includes('transport')) {
      ...
      for (const [index, material] of mentionedMaterials.entries()) {
...
router.post('/', async (req, res) => {
  try {
    const { title, description, vehicleId, priority, originalInstruction } = req.body;

    if (!originalInstruction) {
      return res.status(400).json({ error: 'Original instruction is required' });
    }

    // Decompose the task
    const decomposition = await TaskDecomposer.decomposeTask(originalInstruction, vehicleId);

    const newTask = new Task({
      id: uuidv4(),
      title: title || `Auto-generated: ${originalInstruction.substring(0, 50)}...`,
      description: description || originalInstruction,
      vehicleId: vehicleId || null,
      status: 'pending_approval',
      priority: priority || decomposition.priority,
