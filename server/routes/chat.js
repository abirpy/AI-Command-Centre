...
// Add a new chat message (general endpoint)
router.post('/', async (req, res) => {
  try {
    const { vehicleId, message, sender, messageType = 'text', priority = 'normal' } = req.body

    if (!message || !sender || !vehicleId) {
      return res.status(400).json({ error: 'Message, sender, and vehicleId are required' })
    }
